...
class WebhookController extends Controller
{
    ...
    protected function handleCustomerSubscriptionDeleted(array $payload)
    {
        ...
        if ($user) {
            $user->subscriptions->filter(function ($subscription) use ($payload) {
                return $subscription->zoho_id === $payload['data']['object']['id'];
    ...
    {
        $model = getenv('STRIPE_MODEL') ?: config('services.zoho.model');

        return (new $model)->where('zoho_id', $stripeId)->first();
    }
    ...
    {
        try {
            return ! is_null(StripeEvent::retrieve($id, config('services.zoho.secret')));
        } catch (Exception $e) {
            return false;
        }
    }

...
class Subscription extends Model
{
    ...
    {
        $model = getenv('ZOHO_MODEL') ?: config('services.zoho.model', 'App\\User');
    ...
    {
        $subscriptions = $this->user->asZohoCustomer()->subscriptions;

        if (! $subscriptions) {
            throw new LogicException('The Stripe customer does not have any subscriptions.');
        }

        return $subscriptions->retrieve($this->zoho_id);
    }
}
